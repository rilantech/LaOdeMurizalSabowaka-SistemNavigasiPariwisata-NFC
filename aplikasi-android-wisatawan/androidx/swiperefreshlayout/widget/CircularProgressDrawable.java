    ...
    public void applyTransformation(float interpolatedTime, Ring ring, boolean lastFrame) {
        ...
        if (this.mFinishing) {
            ...
            if (interpolatedTime < 0.5f) {
                ...
                scaledTime = (MATERIAL_INTERPOLATOR.getInterpolation(interpolatedTime / 0.5f) * 0.79f) + 0.01f + startTrim;
            } else {
                float endTrim = ring.getStartingStartTrim() + 0.79f;
                scaledTime = endTrim;
                startTrim = endTrim - (((1.0f - MATERIAL_INTERPOLATOR.getInterpolation((interpolatedTime - 0.5f) / 0.5f)) * 0.79f) + 0.01f);
            }
