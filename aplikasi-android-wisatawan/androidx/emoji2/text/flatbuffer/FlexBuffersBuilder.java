...
public class FlexBuffersBuilder {
    ...
    private Value createVector(int key, int start, int length, boolean typed, boolean fixed, Value keys) {
        ...
        if (!fixed || typed) {
            ...
            for (int i5 = start; i5 < this.stack.size(); i5++) {
                bitWidth = Math.max(bitWidth, this.stack.get(i5).elemWidth(this.bb.writePosition(), i5 + prefixElems));
                if (!typed) {
                    int i6 = start;
                } else if (i5 == start) {
            ...
            if (!fixed || FlexBuffers.isTypedVectorElementType(vectorType)) {
                ...
                if (!typed) {
                    for (int i9 = start; i9 < this.stack.size(); i9++) {
                        this.bb.put(this.stack.get(i9).storedPackedType(bitWidth));
                    }
                }
                if (value != null) {
                    i = 9;
    ...
    private Value createKeyVector(int start, int length) {
        ...
        int byteWidth = align(bitWidth);
        writeInt((long) length, byteWidth);
        int vloc = this.bb.writePosition();
        int i2 = start;
        while (i2 < this.stack.size()) {
            if (this.stack.get(i2).key != -1) {
                writeOffset((long) this.stack.get(i2).key, byteWidth);
                i2++;
            } else {
                throw new AssertionError();
            }
