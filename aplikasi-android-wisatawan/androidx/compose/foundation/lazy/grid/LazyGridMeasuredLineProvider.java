...
public abstract class LazyGridMeasuredLineProvider {
    ...
    public final long m719childConstraintsJhjzzOo$foundation_release(int startSlot, int span) {
        int i;
        if (span == 1) {
        ...
        if (this.isVertical) {
            return Constraints.Companion.m5798fixedWidthOenEA2s(crossAxisSize);
        }
        return Constraints.Companion.m5797fixedHeightOenEA2s(crossAxisSize);
    }

    /* renamed from: itemConstraints-OenEA2s  reason: not valid java name */
    public final long m720itemConstraintsOenEA2s(int itemIndex) {
        LazyGridSpanLayoutProvider lazyGridSpanLayoutProvider = this.spanLayoutProvider;
        return m719childConstraintsJhjzzOo$foundation_release(0, lazyGridSpanLayoutProvider.spanOf(itemIndex, lazyGridSpanLayoutProvider.getSlotsPerLine()));
    }
    ...
    public final LazyGridMeasuredLine getAndMeasure(int lineIndex) {
        ...
        for (int i = 0; i < lineItemsCount; i++) {
            int span = GridItemSpan.m694getCurrentLineSpanimpl(lineConfiguration.getSpans().get(i).m697unboximpl());
            LazyGridMeasuredItem r9 = this.measuredItemProvider.m718getAndMeasure3p2s80s(lineConfiguration.getFirstItemIndex() + i, mainAxisSpacing, m719childConstraintsJhjzzOo$foundation_release(startSlot, span));
