...
public final class MultiWidgetSelectionDelegate implements Selectable {
        ...
        if (this._previousTextLayoutResult != $this$lastVisibleOffset) {
            if ($this$lastVisibleOffset.getDidOverflowHeight()) {
                if (!$this$lastVisibleOffset.getMultiParagraph().getDidExceedMaxLines()) {
                    finalVisibleLine = RangesKt.coerceAtMost($this$lastVisibleOffset.getLineForVerticalPosition((float) IntSize.m6003getHeightimpl($this$lastVisibleOffset.m5298getSizeYbymL2g())), $this$lastVisibleOffset.getLineCount() - 1);
                    while ($this$lastVisibleOffset.getLineTop(finalVisibleLine) >= ((float) IntSize.m6003getHeightimpl($this$lastVisibleOffset.m5298getSizeYbymL2g()))) {
                        finalVisibleLine--;
                    }
    ...
    public Pair<Selection, Boolean> m1048updateSelectionqCDeeow(long startHandlePosition, long endHandlePosition, Offset previousHandlePosition, boolean isStartHandle, LayoutCoordinates containerLayoutCoordinates, SelectionAdjustment adjustment, Selection previousSelection) {
        ...
        if (z) {
            ...
            long relativePosition = layoutCoordinates.m4651localPositionOfR5De75A(layoutCoordinates2, Offset.Companion.m3019getZeroF1C5BW0());
            long localStartPosition = Offset.m3007minusMKHz9U(startHandlePosition, relativePosition);
            long localEndPosition = Offset.m3007minusMKHz9U(endHandlePosition, relativePosition);
            if (previousHandlePosition != null) {
                localPreviousHandlePosition = Offset.m2992boximpl(Offset.m3007minusMKHz9U(previousHandlePosition.m3013unboximpl(), relativePosition));
            }
            long j = relativePosition;
            return MultiWidgetSelectionDelegateKt.m1052getTextSelectionInfoyM0VcXU(textLayoutResult, localStartPosition, localEndPosition, localPreviousHandlePosition, getSelectableId(), adjustment, previousSelection, isStartHandle);
        }
    ...
    public Selection getSelectAllSelection() {
        ...
        return MultiWidgetSelectionDelegateKt.m1050getAssembledSelectionInfovJH6DeI(TextRangeKt.TextRange(0, textLayoutResult.getLayoutInput().getText().length()), false, getSelectableId(), textLayoutResult);
    }

    /* renamed from: getHandlePosition-dBAh8RU  reason: not valid java name */
    public long m1046getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
    ...
    public long m1047getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult textLayoutResult = this.layoutResultCallback.invoke();
        if (textLayoutResult == null) {
