...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return (AndroidFlingSpline.FlingResult.m417getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m412flingPositionLfoxSSI(duration > 0 ? ((float) playTimeNanos) / ((float) duration) : 1.0f)) * flingDistance(initialVelocity)) + initialValue;
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        long duration = getDurationNanos(0.0f, initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m418getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m412flingPositionLfoxSSI(duration > 0 ? ((float) playTimeNanos) / ((float) duration) : 1.0f)) * flingDistance(initialVelocity)) / ((float) duration)) * 1.0E9f;
    }
}
