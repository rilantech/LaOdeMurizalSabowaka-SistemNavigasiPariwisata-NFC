...
final class WrapContentNode extends Modifier.Node implements LayoutModifierNode {
    ...
    public MeasureResult m674measure3p2s80s(MeasureScope $this$measure_u2d3p2s80s, Measurable measurable, long constraints) {
        int i;
        Measurable measurable2 = measurable;
        Intrinsics.checkNotNullParameter($this$measure_u2d3p2s80s, "$this$measure");
        Intrinsics.checkNotNullParameter(measurable2, "measurable");
        int i2 = 0;
        int r0 = this.direction != Direction.Vertical ? 0 : Constraints.m5790getMinWidthimpl(constraints);
        if (this.direction == Direction.Horizontal) {
            i2 = Constraints.m5789getMinHeightimpl(constraints);
        }
        int i3 = Integer.MAX_VALUE;
        if (this.direction == Direction.Vertical || !this.unbounded) {
            i = Constraints.m5788getMaxWidthimpl(constraints);
        } else {
            i = Integer.MAX_VALUE;
        }
        if (this.direction == Direction.Horizontal || !this.unbounded) {
            i3 = Constraints.m5787getMaxHeightimpl(constraints);
        }
        ...
        int wrapperWidth = RangesKt.coerceIn(placeable.getWidth(), Constraints.m5790getMinWidthimpl(constraints), Constraints.m5788getMaxWidthimpl(constraints));
        int wrapperHeight = RangesKt.coerceIn(placeable.getHeight(), Constraints.m5789getMinHeightimpl(constraints), Constraints.m5787getMaxHeightimpl(constraints));
