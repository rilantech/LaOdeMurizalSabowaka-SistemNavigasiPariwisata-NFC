...
public final class BringIntoViewRequestPriorityQueue {
    ...
    public final boolean enqueue(ContentInViewModifier.Request request) {
        ...
        if (requestBounds == null) {
            ...
            request.getContinuation().resumeWith(Result.m6229constructorimpl(Unit.INSTANCE));
            return false;
        }
    ...
    public final void resumeAndRemoveAll() {
        ...
        if (i <= last) {
            while (true) {
                ...
                Result.Companion companion = Result.Companion;
                ((ContentInViewModifier.Request) this.requests.getContent()[i]).getContinuation().resumeWith(Result.m6229constructorimpl(unit));
                if (i == last) {
                    break;
                }
                i++;
    ...
    public final void resumeAndRemoveWhile(Function1<? super Rect, Boolean> block) {
        ...
        while (this.requests.isNotEmpty() && block.invoke(((ContentInViewModifier.Request) this.requests.last()).getCurrentBounds().invoke()).booleanValue()) {
            Unit unit = Unit.INSTANCE;
            Result.Companion companion = Result.Companion;
            ((ContentInViewModifier.Request) this.requests.removeAt(this.requests.getSize() - 1)).getContinuation().resumeWith(Result.m6229constructorimpl(unit));
        }
    }
