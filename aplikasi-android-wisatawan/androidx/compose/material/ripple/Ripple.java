...
public abstract class Ripple implements Indication {
    ...
    public abstract RippleIndicationInstance m1515rememberUpdatedRippleInstance942rkJo(InteractionSource interactionSource, boolean z, float f, State<Color> state, State<RippleAlpha> state2, Composer composer, int i);

    private Ripple(boolean bounded2, float radius2, State<Color> color2) {
        Intrinsics.checkNotNullParameter(color2, TypedValues.Custom.S_COLOR);
    ...
    public final IndicationInstance rememberUpdatedInstance(InteractionSource interactionSource, Composer $composer, int $changed) {
        ...
        if (this.color.getValue().m3263unboximpl() != Color.Companion.m3289getUnspecified0d7_KjU()) {
            j = this.color.getValue().m3263unboximpl();
        } else {
            j = theme.m1524defaultColorWaAFU9c(composer, 0);
        }
        $composer.endReplaceableGroup();
        RippleIndicationInstance instance = m1515rememberUpdatedRippleInstance942rkJo(interactionSource, this.bounded, this.radius, SnapshotStateKt.rememberUpdatedState(Color.m3243boximpl(j), composer, 0), SnapshotStateKt.rememberUpdatedState(theme.rippleAlpha(composer, 0), composer, 0), $composer, (i & 14) | ((i << 12) & 458752));
        EffectsKt.LaunchedEffect(instance, interactionSource2, new Ripple$rememberUpdatedInstance$1(interactionSource2, instance, (Continuation<? super Ripple$rememberUpdatedInstance$1>) null), composer, ((i << 3) & 112) | 520);
        if (ComposerKt.isTraceInProgress()) {
    ...
    public boolean equals(Object other) {
        ...
        if ((other instanceof Ripple) && this.bounded == ((Ripple) other).bounded && Dp.m5849equalsimpl0(this.radius, ((Ripple) other).radius) && Intrinsics.areEqual((Object) this.color, (Object) ((Ripple) other).color)) {
            return true;
        }
        return false;
    }
    ...
    public int hashCode() {
        return (((Boolean.hashCode(this.bounded) * 31) + Dp.m5850hashCodeimpl(this.radius)) * 31) + this.color.hashCode();
    }
}
