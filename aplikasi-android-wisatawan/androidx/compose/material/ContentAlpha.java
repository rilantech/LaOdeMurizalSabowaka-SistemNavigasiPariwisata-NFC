...
public final class ContentAlpha {
    ...
    private final float contentAlpha(float highContrastAlpha, float lowContrastAlpha, Composer $composer, int $changed) {
        ...
        long contentColor = ((Color) consume).m3263unboximpl();
        float f = (!MaterialTheme.INSTANCE.getColors($composer, 6).isLight() ? ((double) ColorKt.m3305luminance8_81llA(contentColor)) >= 0.5d : ((double) ColorKt.m3305luminance8_81llA(contentColor)) <= 0.5d) ? lowContrastAlpha : highContrastAlpha;
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        $composer.endReplaceableGroup();
