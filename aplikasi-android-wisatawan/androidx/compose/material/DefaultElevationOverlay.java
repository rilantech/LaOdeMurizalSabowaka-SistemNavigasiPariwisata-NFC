...
final class DefaultElevationOverlay implements ElevationOverlay {
    ...
    public long m1275apply7g2Lkgo(long color, float elevation, Composer $composer, int $changed) {
        ...
        if (Dp.m5843compareTo0680j_4(elevation, Dp.m5844constructorimpl((float) 0)) <= 0 || colors.isLight()) {
            foregroundColor = color;
        } else {
            foregroundColor = ColorKt.m3298compositeOverOWjLjI(ElevationOverlayKt.m1306calculateForegroundColorCLU3JFs(color, elevation, $composer, ($changed & 14) | ($changed & 112)), color);
        }
        if (ComposerKt.isTraceInProgress()) {
