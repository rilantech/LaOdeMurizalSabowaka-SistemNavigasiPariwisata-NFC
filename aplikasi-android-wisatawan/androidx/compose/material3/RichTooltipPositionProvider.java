...
final class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public RichTooltipPositionProvider(int tooltipAnchorPadding2) {
        this.tooltipAnchorPadding = tooltipAnchorPadding2;
    }
    ...
    public long m1936calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        Intrinsics.checkNotNullParameter(anchorBounds, "anchorBounds");
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        int x = anchorBounds.getRight();
        if (IntSize.m6004getWidthimpl(popupContentSize) + x > IntSize.m6004getWidthimpl(windowSize) && (x = anchorBounds.getLeft() - IntSize.m6004getWidthimpl(popupContentSize)) < 0) {
            x = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m6004getWidthimpl(popupContentSize)) / 2);
        }
        int y = (anchorBounds.getTop() - IntSize.m6003getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (y < 0) {
