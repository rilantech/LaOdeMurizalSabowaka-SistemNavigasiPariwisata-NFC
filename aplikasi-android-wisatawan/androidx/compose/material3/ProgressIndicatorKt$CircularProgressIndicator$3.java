...
final class ProgressIndicatorKt$CircularProgressIndicator$3 extends Lambda implements Function1<DrawScope, Unit> {
    ...
    public final void invoke(DrawScope $this$Canvas) {
        ...
        float f = sweep;
        ProgressIndicatorKt.m1928drawIndeterminateCircularIndicatorhrjfTZI(drawScope, this.$startAngle.getValue().floatValue() + -90.0f + ((this.$currentRotation.getValue().floatValue() * 216.0f) % 360.0f) + this.$baseRotation.getValue().floatValue(), this.$strokeWidth, f, this.$color, this.$stroke);
    }
}
