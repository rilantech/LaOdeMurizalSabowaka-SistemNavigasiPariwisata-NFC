    ...
    public final void invoke(Composer $composer, int $changed) {
        ...
        if (($changed & 11) != 2 || !$composer.getSkipping()) {
            ...
            if (this.$topBarPlaceables.isEmpty()) {
                f = insets.m555calculateTopPaddingD9Ej5fM();
            } else {
                f = this.$this_SubcomposeLayout.m5822toDpu2uoSUM(this.$topBarHeight);
            }
            if (this.$bottomBarPlaceables.isEmpty() || (num = this.$bottomBarHeight) == null) {
                f2 = insets.m552calculateBottomPaddingD9Ej5fM();
            } else {
                f2 = this.$this_SubcomposeLayout.m5822toDpu2uoSUM(num.intValue());
            }
            this.$content.invoke(PaddingKt.m534PaddingValuesa9UjIt4(PaddingKt.calculateStartPadding(insets, this.$this_SubcomposeLayout.getLayoutDirection()), f, PaddingKt.calculateEndPadding(insets, this.$this_SubcomposeLayout.getLayoutDirection()), f2), $composer, Integer.valueOf((this.$$dirty >> 3) & 112));
            if (ComposerKt.isTraceInProgress()) {
