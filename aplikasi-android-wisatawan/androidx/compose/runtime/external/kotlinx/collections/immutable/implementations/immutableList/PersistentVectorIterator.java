...
public final class PersistentVectorIterator<T> extends AbstractListIterator<T> {
    ...
    public PersistentVectorIterator(Object[] root, T[] tail2, int index, int size, int trieHeight) {
        super(index, size);
        Intrinsics.checkNotNullParameter(root, "root");
        Intrinsics.checkNotNullParameter(tail2, "tail");
        this.tail = tail2;
        int trieSize = UtilsKt.rootSize(size);
        this.trieIterator = new TrieIterator<>(root, RangesKt.coerceAtMost(index, trieSize), trieSize, trieHeight);
    }
