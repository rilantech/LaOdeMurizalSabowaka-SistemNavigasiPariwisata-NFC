...
public class PersistentHashMapBuilderBaseIterator<K, V, T> extends PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMutableIterator {
    ...
    public PersistentHashMapBuilderBaseIterator(PersistentHashMapBuilder<K, V> builder2, TrieNodeBaseIterator<K, V, T>[] path) {
        super(builder2.getNode$runtime_release(), path);
    ...
    private final void resetPath(int keyHash, TrieNode<?, ?> node, K key, int pathIndex) {
        ...
        if (shift > 30) {
            getPath()[pathIndex].reset(node.getBuffer$runtime_release(), node.getBuffer$runtime_release().length, 0);
            while (!Intrinsics.areEqual(getPath()[pathIndex].currentKey(), (Object) key)) {
        ...
        if (node.hasEntryAt$runtime_release(keyPositionMask)) {
            getPath()[pathIndex].reset(node.getBuffer$runtime_release(), node.entryCount$runtime_release() * 2, node.entryKeyIndex$runtime_release(keyPositionMask));
            setPathLastIndex(pathIndex);
            return;
        }
        ...
        getPath()[pathIndex].reset(node.getBuffer$runtime_release(), node.entryCount$runtime_release() * 2, nodeIndex);
