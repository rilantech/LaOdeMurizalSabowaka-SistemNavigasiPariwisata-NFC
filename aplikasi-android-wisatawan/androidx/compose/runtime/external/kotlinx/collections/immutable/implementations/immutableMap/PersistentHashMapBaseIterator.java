...
public abstract class PersistentHashMapBaseIterator<K, V, T> implements Iterator<T>, KMappedMarker {
    ...
    public PersistentHashMapBaseIterator(TrieNode<K, V> node, TrieNodeBaseIterator<K, V, T>[] path2) {
        Intrinsics.checkNotNullParameter(node, "node");
        Intrinsics.checkNotNullParameter(path2, "path");
        this.path = path2;
        path2[0].reset(node.getBuffer$runtime_release(), node.entryCount$runtime_release() * 2);
    ...
    private final int moveToNextNodeWithData(int pathIndex) {
        ...
        if (pathIndex == 6) {
            this.path[pathIndex + 1].reset(node.getBuffer$runtime_release(), node.getBuffer$runtime_release().length);
        } else {
            this.path[pathIndex + 1].reset(node.getBuffer$runtime_release(), node.entryCount$runtime_release() * 2);
        }
    ...
    public T next() {
        checkHasNext();
        Object result = this.path[this.pathLastIndex].next();
        ensureNextEntryIsReady();
        return result;
    }
