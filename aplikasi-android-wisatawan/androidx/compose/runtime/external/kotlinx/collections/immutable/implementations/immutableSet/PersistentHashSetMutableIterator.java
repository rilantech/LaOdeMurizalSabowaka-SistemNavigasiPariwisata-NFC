...
public final class PersistentHashSetMutableIterator<E> extends PersistentHashSetIterator<E> implements Iterator<E>, KMutableIterator {
    ...
    public PersistentHashSetMutableIterator(PersistentHashSetBuilder<E> builder2) {
        super(builder2.getNode$runtime_release());
    ...
    private final void resetPath(int hashCode, TrieNode<?> node, E element, int pathIndex) {
        boolean z = true;
        if (isCollision(node)) {
            int index = ArraysKt.indexOf((T[]) node.getBuffer(), element);
            if (index == -1) {
                z = false;
            }
            CommonFunctionsKt.m2881assert(z);
            ((TrieNodeIterator) getPath().get(pathIndex)).reset(node.getBuffer(), index);
            setPathLastIndex(pathIndex);
            return;
        }
        int index2 = node.indexOfCellAt$runtime_release(1 << TrieNodeKt.indexSegment(hashCode, pathIndex * 5));
        ((TrieNodeIterator) getPath().get(pathIndex)).reset(node.getBuffer(), index2);
