...
public class SnapshotMutableDoubleStateImpl implements StateObject, MutableDoubleState, SnapshotMutableState<Double> {
    ...
    public void setDoubleValue(double value) {
        Snapshot current;
        Unit unit;
        DoubleStateStateRecord it = (DoubleStateStateRecord) SnapshotKt.current(this.next);
        if (!(it.getValue() == value)) {
            StateRecord $this$overwritable$iv = this.next;
            Object snapshot$iv = SnapshotKt.getSnapshotInitializer();
            synchronized (SnapshotKt.getLock()) {
                current = Snapshot.Companion.getCurrent();
                ((DoubleStateStateRecord) SnapshotKt.overwritableRecord($this$overwritable$iv, this, current, it)).setValue(value);
                unit = Unit.INSTANCE;
            }
            Unit unit2 = unit;
    ...
    private static final class DoubleStateStateRecord extends StateRecord {
        ...
        public DoubleStateStateRecord(double value2) {
            this.value = value2;
        }

        public void assign(StateRecord value2) {
            Intrinsics.checkNotNullParameter(value2, "value");
            this.value = ((DoubleStateStateRecord) value2).value;
        }
