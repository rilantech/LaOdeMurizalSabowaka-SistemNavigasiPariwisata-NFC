...
final class Pending {
    ...
    public Pending(List<KeyInfo> keyInfos2, int startIndex2) {
        Intrinsics.checkNotNullParameter(keyInfos2, "keyInfos");
        this.keyInfos = keyInfos2;
        ...
        if (startIndex2 >= 0) {
            ...
            for (int index = 0; index < size; index++) {
                ...
                hashMap.put(Integer.valueOf(keyInfo.getLocation()), new GroupInfo(index, runningNodeIndex, keyInfo.getNodes()));
    ...
    public final void registerMoveSlot(int from, int to) {
        if (from > to) {
            Collection<GroupInfo> $this$forEach$iv = this.groupInfos.values();
            Intrinsics.checkNotNullExpressionValue($this$forEach$iv, "groupInfos.values");
            for (GroupInfo group : $this$forEach$iv) {
            ...
            Collection<GroupInfo> $this$forEach$iv2 = this.groupInfos.values();
            Intrinsics.checkNotNullExpressionValue($this$forEach$iv2, "groupInfos.values");
            for (GroupInfo group2 : $this$forEach$iv2) {
    ...
    public final boolean updateNodeCount(int group, int newCount) {
        ...
        Collection<GroupInfo> $this$forEach$iv = this.groupInfos.values();
        Intrinsics.checkNotNullExpressionValue($this$forEach$iv, "groupInfos.values");
        for (GroupInfo childGroupInfo : $this$forEach$iv) {
            if (childGroupInfo.getNodeIndex() >= index && !Intrinsics.areEqual((Object) childGroupInfo, (Object) groupInfo) && (newIndex = childGroupInfo.getNodeIndex() + difference) >= 0) {
