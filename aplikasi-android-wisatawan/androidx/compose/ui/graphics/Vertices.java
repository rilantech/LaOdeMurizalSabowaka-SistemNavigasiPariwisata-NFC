...
public final class Vertices {
    ...
    public final int m3691getVertexModec2xauaI() {
    ...
    private final int[] encodeColorList(List<Color> colors2) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3307toArgb8_81llA(colors2.get(i).m3263unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            long point = points.get(i / 2).m3013unboximpl();
            if (i % 2 == 0) {
                f = Offset.m3003getXimpl(point);
            } else {
                f = Offset.m3004getYimpl(point);
            }
            fArr[i] = f;
        }
        return fArr;
