...
public final class AndroidPath implements Path {
    ...
    public int m3135getFillTypeRgk1Os() {
        if (this.internalPath.getFillType() == Path.FillType.EVEN_ODD) {
            return PathFillType.Companion.m3551getEvenOddRgk1Os();
        }
        return PathFillType.Companion.m3552getNonZeroRgk1Os();
    }

    /* renamed from: setFillType-oQ8Xj4U  reason: not valid java name */
    public void m3137setFillTypeoQ8Xj4U(int value) {
        Path.FillType fillType;
        Path path = this.internalPath;
        if (PathFillType.m3547equalsimpl0(value, PathFillType.Companion.m3551getEvenOddRgk1Os())) {
    ...
    public void addRoundRect(RoundRect roundRect) {
        ...
        this.radii[0] = CornerRadius.m2978getXimpl(roundRect.m3053getTopLeftCornerRadiuskKHJgLs());
        this.radii[1] = CornerRadius.m2979getYimpl(roundRect.m3053getTopLeftCornerRadiuskKHJgLs());
        this.radii[2] = CornerRadius.m2978getXimpl(roundRect.m3054getTopRightCornerRadiuskKHJgLs());
        this.radii[3] = CornerRadius.m2979getYimpl(roundRect.m3054getTopRightCornerRadiuskKHJgLs());
        this.radii[4] = CornerRadius.m2978getXimpl(roundRect.m3052getBottomRightCornerRadiuskKHJgLs());
        this.radii[5] = CornerRadius.m2979getYimpl(roundRect.m3052getBottomRightCornerRadiuskKHJgLs());
        this.radii[6] = CornerRadius.m2978getXimpl(roundRect.m3051getBottomLeftCornerRadiuskKHJgLs());
        this.radii[7] = CornerRadius.m2979getYimpl(roundRect.m3051getBottomLeftCornerRadiuskKHJgLs());
        this.internalPath.addRoundRect(this.rectF, this.radii, Path.Direction.CCW);
    }

    /* renamed from: addPath-Uv8p0NA  reason: not valid java name */
    public void m3134addPathUv8p0NA(Path path, long offset) {
        Intrinsics.checkNotNullParameter(path, "path");
        Path path2 = this.internalPath;
        Path $this$asAndroidPath$iv = path;
        if ($this$asAndroidPath$iv instanceof AndroidPath) {
            path2.addPath(((AndroidPath) $this$asAndroidPath$iv).getInternalPath(), Offset.m3003getXimpl(offset), Offset.m3004getYimpl(offset));
            return;
        }
    ...
    public void m3139translatek4lQ0M(long offset) {
        this.mMatrix.reset();
        this.mMatrix.setTranslate(Offset.m3003getXimpl(offset), Offset.m3004getYimpl(offset));
        this.internalPath.transform(this.mMatrix);
    }

    /* renamed from: transform-58bKbWc  reason: not valid java name */
    public void m3138transform58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        AndroidMatrixConversions_androidKt.m3114setFromEL8BTi8(this.mMatrix, matrix);
        this.internalPath.transform(this.mMatrix);
    }
    ...
    public boolean m3136opN5in7k0(Path path1, Path path2, int operation) {
        Path.Op op;
        Intrinsics.checkNotNullParameter(path1, "path1");
        Intrinsics.checkNotNullParameter(path2, "path2");
        if (PathOperation.m3558equalsimpl0(operation, PathOperation.Companion.m3562getDifferenceb3I0S0c())) {
            op = Path.Op.DIFFERENCE;
        } else if (PathOperation.m3558equalsimpl0(operation, PathOperation.Companion.m3563getIntersectb3I0S0c())) {
            op = Path.Op.INTERSECT;
        } else if (PathOperation.m3558equalsimpl0(operation, PathOperation.Companion.m3564getReverseDifferenceb3I0S0c())) {
            op = Path.Op.REVERSE_DIFFERENCE;
        } else if (PathOperation.m3558equalsimpl0(operation, PathOperation.Companion.m3565getUnionb3I0S0c())) {
            op = Path.Op.UNION;
        } else {
            op = Path.Op.XOR;
        }
