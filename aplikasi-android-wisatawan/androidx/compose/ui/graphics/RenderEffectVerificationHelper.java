...
final class RenderEffectVerificationHelper {
    ...
    public final RenderEffect m3584createBlurEffect8A3gB4(RenderEffect inputRenderEffect, float radiusX, float radiusY, int edgeTreatment) {
        if (inputRenderEffect == null) {
            RenderEffect createBlurEffect = RenderEffect.createBlurEffect(radiusX, radiusY, AndroidTileMode_androidKt.m3149toAndroidTileMode0vamqd0(edgeTreatment));
            Intrinsics.checkNotNullExpressionValue(createBlurEffect, "{\n            android.gr…)\n            )\n        }");
            return createBlurEffect;
        }
        RenderEffect createBlurEffect2 = RenderEffect.createBlurEffect(radiusX, radiusY, inputRenderEffect.asAndroidRenderEffect(), AndroidTileMode_androidKt.m3149toAndroidTileMode0vamqd0(edgeTreatment));
    ...
    public final RenderEffect m3585createOffsetEffectUv8p0NA(RenderEffect inputRenderEffect, long offset) {
        if (inputRenderEffect == null) {
            RenderEffect createOffsetEffect = RenderEffect.createOffsetEffect(Offset.m3003getXimpl(offset), Offset.m3004getYimpl(offset));
            Intrinsics.checkNotNullExpressionValue(createOffsetEffect, "{\n            android.gr…et.x, offset.y)\n        }");
            return createOffsetEffect;
        }
        RenderEffect createOffsetEffect2 = RenderEffect.createOffsetEffect(Offset.m3003getXimpl(offset), Offset.m3004getYimpl(offset), inputRenderEffect.asAndroidRenderEffect());
