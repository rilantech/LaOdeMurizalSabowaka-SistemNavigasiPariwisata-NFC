...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m5113toOffsetUv8p0NA(int[] consumed, long available) {
        float offsetX;
        float offsetY;
        if (Offset.m3003getXimpl(available) >= 0.0f) {
            offsetX = RangesKt.coerceAtMost(reverseAxis(consumed[0]), Offset.m3003getXimpl(available));
        } else {
            offsetX = RangesKt.coerceAtLeast(reverseAxis(consumed[0]), Offset.m3003getXimpl(available));
        }
        if (Offset.m3004getYimpl(available) >= 0.0f) {
            offsetY = RangesKt.coerceAtMost(reverseAxis(consumed[1]), Offset.m3004getYimpl(available));
        } else {
            offsetY = RangesKt.coerceAtLeast(reverseAxis(consumed[1]), Offset.m3004getYimpl(available));
        }
    ...
    public static final int m5114toViewTypeGyEprt8(int $this$toViewType_u2dGyEprt8) {
        if (NestedScrollSource.m4372equalsimpl0($this$toViewType_u2dGyEprt8, NestedScrollSource.Companion.m4377getDragWNlRxjI())) {
            return 0;
        }
        return 1;
    }
    ...
    public static final int m5112getScrollAxesk4lQ0M(long $this$scrollAxes) {
        int axes = 0;
        if (Math.abs(Offset.m3003getXimpl($this$scrollAxes)) >= 0.5f) {
            axes = 0 | 1;
        }
        if (Math.abs(Offset.m3004getYimpl($this$scrollAxes)) >= 0.5f) {
            return axes | 2;
        }
        return axes;
    }
