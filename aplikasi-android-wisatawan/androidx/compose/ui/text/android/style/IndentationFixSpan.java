...
public final class IndentationFixSpan implements LeadingMarginSpan {
    ...
    public void drawLeadingMargin(Canvas canvas, Paint paint, int left, int dir, int top, int baseline, int bottom, CharSequence text, int start, int end, boolean first, Layout layout) {
        ...
        if (layout2 == null || paint2 == null) {
            Canvas canvas2 = canvas;
            int i = start;
            return;
        }
        int lineIndex = layout2.getLineForOffset(start);
        boolean z = true;
        if (lineIndex != layout.getLineCount() - 1 || !TextLayoutKt.isLineEllipsized(layout2, lineIndex)) {
            Canvas canvas3 = canvas;
            return;
        }
        float padding = IndentationFixSpanKt.getEllipsizedLeftPadding(layout2, lineIndex, paint) + IndentationFixSpanKt.getEllipsizedRightPadding(layout2, lineIndex, paint);
        if (padding != 0.0f) {
            z = false;
        }
        if (!z) {
