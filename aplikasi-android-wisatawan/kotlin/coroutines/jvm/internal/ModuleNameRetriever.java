...
final class ModuleNameRetriever {
    ...
    private final Cache buildCache(BaseContinuationImpl continuation) {
        try {
            Cache it = new Cache(Class.class.getDeclaredMethod("getModule", new Class[0]), continuation.getClass().getClassLoader().loadClass("java.lang.Module").getDeclaredMethod("getDescriptor", new Class[0]), continuation.getClass().getClassLoader().loadClass("java.lang.module.ModuleDescriptor").getDeclaredMethod(HintConstants.AUTOFILL_HINT_NAME, new Class[0]));
            cache = it;
            return it;
        } catch (Exception e) {
