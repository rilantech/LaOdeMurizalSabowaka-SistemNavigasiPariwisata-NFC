...
final class UIntProgressionIterator implements Iterator<UInt>, KMappedMarker {
    ...
    private UIntProgressionIterator(int first, int last, int step2) {
        ...
        if (step2 <= 0 ? compareUnsigned < 0 : compareUnsigned > 0) {
            z = false;
        }
        this.hasNext = z;
        this.step = UInt.m6324constructorimpl(step2);
        this.next = this.hasNext ? first : last;
    }

    public /* bridge */ /* synthetic */ Object next() {
        return UInt.m6318boximpl(m7450nextpVg5ArA());
    }
    ...
    public int m7450nextpVg5ArA() {
        int value = this.next;
        if (value != this.finalElement) {
            this.next = UInt.m6324constructorimpl(this.next + this.step);
