...
final class ULongProgressionIterator implements Iterator<ULong>, KMappedMarker {
    ...
    private ULongProgressionIterator(long first, long last, long step2) {
        ...
        boolean z = true;
        if (step2 <= 0 ? Long.compareUnsigned(first, last) < 0 : Long.compareUnsigned(first, last) > 0) {
            z = false;
        }
        this.hasNext = z;
        this.step = ULong.m6403constructorimpl(step2);
        this.next = this.hasNext ? first : last;
    }

    public /* bridge */ /* synthetic */ Object next() {
        return ULong.m6397boximpl(m7459nextsVKNKU());
    }
    ...
    public long m7459nextsVKNKU() {
        long value = this.next;
        if (value != this.finalElement) {
            this.next = ULong.m6403constructorimpl(this.next + this.step);
