...
public class ProgressWheel extends View {
    ...
    private void setupBounds(int layout_width, int layout_height) {
        ...
        if (!this.fillRadius) {
            ...
            int i = this.barWidth;
            this.circleBounds = new RectF((float) (xOffset + i), (float) (yOffset + i), (float) ((xOffset + circleDiameter) - i), (float) ((yOffset + circleDiameter) - i));
            return;
        }
        int i2 = this.barWidth;
        this.circleBounds = new RectF((float) (paddingLeft + i2), (float) (paddingTop + i2), (float) ((layout_width - paddingRight) - i2), (float) ((layout_height - paddingBottom) - i2));
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        this.spinSpeed = 360.0f * a.getFloat(R.styleable.ProgressWheel_matProg_spinSpeed, this.spinSpeed / 360.0f);
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (this.shouldAnimate) {
            if (this.isSpinning) {
                ...
                float f = this.mProgress + ((((float) deltaTime) * this.spinSpeed) / 1000.0f);
                this.mProgress = f;
                if (f > 360.0f) {
                ...
                if (this.mProgress != this.mTargetProgress) {
                    mustInvalidate = true;
                    float f2 = this.spinSpeed;
                    this.mProgress = Math.min(this.mProgress + (f2 * (((float) (SystemClock.uptimeMillis() - this.lastTimeAnimated)) / 1000.0f)), this.mTargetProgress);
                    this.lastTimeAnimated = SystemClock.uptimeMillis();
                }
    ...
    private void updateBarLength(long deltaTimeInMilliSeconds) {
        ...
        if (j >= 200) {
            double d = this.timeStartGrowing + ((double) deltaTimeInMilliSeconds);
            ...
            float distance = (((float) Math.cos(((this.timeStartGrowing / d2) + 1.0d) * 3.141592653589793d)) / 2.0f) + 0.5f;
    ...
    static class WheelSavedState extends View.BaseSavedState {
        ...
        private WheelSavedState(Parcel in) {
            ...
            this.fillRadius = in.readByte() == 0 ? false : z;
        }
