...
public final class zzbb implements FusedLocationProviderApi {
    ...
    public final Location getLastLocation(GoogleApiClient googleApiClient) {
        ...
        zzdu zzdu = (zzdu) googleApiClient.getClient(zzbi.zza);
        AtomicReference atomicReference = new AtomicReference();
        CountDownLatch countDownLatch = new CountDownLatch(1);
        TaskCompletionSource taskCompletionSource = new TaskCompletionSource();
        try {
            zzdu.zzq(new LastLocationRequest.Builder().build(), taskCompletionSource);
            taskCompletionSource.getTask().addOnCompleteListener(new zzbe(atomicReference, countDownLatch));
            if (zzey.zza(countDownLatch, 30, TimeUnit.SECONDS)) {
    ...
    public final LocationAvailability getLocationAvailability(GoogleApiClient googleApiClient) {
        ...
        TaskCompletionSource taskCompletionSource = new TaskCompletionSource();
        try {
            zzdu.zzp(zzac.zza(), taskCompletionSource);
