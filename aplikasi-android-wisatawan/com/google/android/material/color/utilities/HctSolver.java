...
public class HctSolver {
    ...
    static int findResultByJ(double hueRadians, double chroma, double y) {
        ...
        while (iterationRound < 5) {
            ...
            double eHue2 = eHue;
            double p2 = (viewingConditions.getAw() * Math.pow(jNormalized, (1.0d / viewingConditions.getC()) / viewingConditions.getZ())) / viewingConditions.getNbb();
            double gamma = (((p2 + 0.305d) * 23.0d) * t) / (((23.0d * p1) + ((t * 11.0d) * hCos)) + ((108.0d * t) * hSin));
            double a = gamma * hCos;
            double b = gamma * hSin;
            double d = jNormalized;
            ...
            double[] dArr = Y_FROM_LINRGB;
            double fnj = (linrgb[0] * dArr[0]) + (linrgb[1] * dArr[1]) + (linrgb[2] * dArr[2]);
            if (fnj <= 0.0d) {
                return 0;
            }
