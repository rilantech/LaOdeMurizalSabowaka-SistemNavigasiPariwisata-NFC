...
<input type="text" placeholder="Cari Kategori..." name="search" class="form-control search-input" value="{{ $search }}" id="search" autofocus>
      ...
      <div class="pb-3 text-center">
        ...
        <div class="row">
            <div class="col-2 pb-3">
                <div class="col-sm-10"><button onclick="window.location='{{ url('tambahkategoriwisata') }}'" type="submit" class="btn btn-info text-white" name="submit">Tambah</button></div>
         ...
         @endif
          ...
          <table class="table text-center table-bordered border-dark table-hover">
              ...
              <tbody>
                ...
                @php
                    $nomor = 1 + (($kategoriwisata->currentPage()-1)* $kategoriwisata->perPage());
                @endphp
                   @foreach ($kategoriwisata as $row)
                    <tr>
                           ...
                           <td>{{ $row->keterangan }}</td>
                            ...
                            <div class="row">
                              @can('admin')
                              <div class="col-6">
                               <button onclick="window.location='{{ url('tabeldaftarkategoriwisata/'.$row->id) }}'" type="button" class="btn btn-md btn-warning" title="Edit Data">
                                    <i class="fa fa-edit"></i>
                               </button>
                              </div>
                              <div class="col-6">
                               <form onsubmit="return deleteData('{{ $row->nama }}')"  action="{{ url('tabeldaftarkategoriwisata/'.$row->id) }}" method="POST"> 
                                @csrf
                                @method('DELETE')
           ...
           {!! $kategoriwisata->appends(Request::except('page'))->render() !!}
