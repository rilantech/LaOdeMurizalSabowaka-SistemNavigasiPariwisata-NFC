...
public final class LocalFullyDrawnReporterOwner {
    ...
    private static final ProvidableCompositionLocal<FullyDrawnReporterOwner> LocalFullyDrawnReporterOwner = CompositionLocalKt.compositionLocalOf$default((SnapshotMutationPolicy) null, LocalFullyDrawnReporterOwner$LocalFullyDrawnReporterOwner$1.INSTANCE, 1, (Object) null);
    ...
    public final FullyDrawnReporterOwner getCurrent(Composer $composer, int $changed) {
        ...
        if (fullyDrawnReporterOwner == null) {
            ...
            ComposerKt.sourceInformationMarkerEnd($composer);
            Context innerContext$iv = (Context) consume3;
            while (true) {
                if (!(innerContext$iv instanceof ContextWrapper)) {
                    innerContext$iv = null;
                    break;
                } else if (innerContext$iv instanceof FullyDrawnReporterOwner) {
                    break;
                } else {
                    innerContext$iv = ((ContextWrapper) innerContext$iv).getBaseContext();
                }
            }
            fullyDrawnReporterOwner = innerContext$iv;
        }
        $composer.endReplaceableGroup();
