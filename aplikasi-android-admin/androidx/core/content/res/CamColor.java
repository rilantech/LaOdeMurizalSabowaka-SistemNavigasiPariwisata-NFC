...
public class CamColor {
    ...
    static CamColor fromColor(int color) {
        ...
        float f = outM3HCT[0];
        float f2 = outM3HCT[1];
        float f3 = outCamColor[0];
        float f4 = outCamColor[1];
        float f5 = outCamColor[2];
        return new CamColor(f, f2, f3, f4, f5, outCamColor[3], outCamColor[4], outCamColor[5], outCamColor[6]);
    }
    ...
    static void fromColorInViewingConditions(int color, ViewingConditions viewingConditions, float[] outCamColor, float[] outM3HCT) {
        float hue;
        float[] fArr = outM3HCT;
        CamUtils.xyzFromInt(color, fArr);
        float[] xyz = outM3HCT;
        ...
        if (atanDegrees < 0.0f) {
            hue = atanDegrees + 360.0f;
        } else {
            hue = atanDegrees >= 360.0f ? atanDegrees - 360.0f : atanDegrees;
        }
        float hue2 = hue;
