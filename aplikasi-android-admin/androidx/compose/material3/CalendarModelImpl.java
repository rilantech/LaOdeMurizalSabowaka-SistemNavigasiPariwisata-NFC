...
public final class CalendarModelImpl implements CalendarModel {
    ...
    public CalendarModelImpl() {
        ...
        for (DayOfWeek it : values) {
            destination$iv$iv.add(TuplesKt.to(it.getDisplayName(TextStyle.FULL, $this$weekdayNames_u24lambda_u241), it.getDisplayName(TextStyle.NARROW, $this$weekdayNames_u24lambda_u241)));
        }
    ...
    public CalendarMonth plusMonths(CalendarMonth from, int addedMonthsCount) {
        ...
        LocalDate laterMonth = toLocalDate(from).plusMonths((long) addedMonthsCount);
    ...
    public CalendarMonth minusMonths(CalendarMonth from, int subtractedMonthsCount) {
        ...
        LocalDate earlierMonth = toLocalDate(from).minusMonths((long) subtractedMonthsCount);
    ...
    private final CalendarMonth getMonth(LocalDate firstDayLocalDate) {
        ...
        return new CalendarMonth(firstDayLocalDate.getYear(), firstDayLocalDate.getMonthValue(), firstDayLocalDate.lengthOfMonth(), daysFromStartOfWeekToFirstOfMonth, firstDayLocalDate.atTime(LocalTime.MIDNIGHT).atZone(utcTimeZoneId).toInstant().toEpochMilli());
    }
