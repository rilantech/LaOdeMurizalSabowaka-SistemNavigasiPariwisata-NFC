...
public final class SheetDefaultsKt$ConsumeSwipeWithinBottomSheetBoundsNestedScrollConnection$1 implements NestedScrollConnection {
    ...
    public long m3675onPreScrollOzD1aCk(long available, int source) {
    ...
    public long m3673onPostScrollDzOQY0M(long consumed, long available, int source) {
    ...
    public Object m3674onPreFlingQWom1Mo(long available, Continuation<? super Velocity> $completion) {
        long j;
        ...
        if (toFling >= 0.0f || currentOffset <= this.$sheetState.getSwipeableState$material3_release().getMinOffset()) {
            j = Velocity.Companion.m7790getZero9UxMQ8M();
        } else {
            this.$onFling.invoke(Boxing.boxFloat(toFling));
            j = available;
        }
        return Velocity.m7770boximpl(j);
    }

    /* renamed from: onPostFling-RZ2iAVY  reason: not valid java name */
    public Object m3672onPostFlingRZ2iAVY(long consumed, long available, Continuation<? super Velocity> $completion) {
        this.$onFling.invoke(Boxing.boxFloat(velocityToFloat(available)));
        return Velocity.m7770boximpl(available);
    }
