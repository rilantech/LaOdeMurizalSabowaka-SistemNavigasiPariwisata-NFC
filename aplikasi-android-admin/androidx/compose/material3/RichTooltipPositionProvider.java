...
final class RichTooltipPositionProvider implements PopupPositionProvider {
    ...
    public long m3646calculatePositionllwVHH4(IntRect anchorBounds, long windowSize, LayoutDirection layoutDirection, long popupContentSize) {
        Intrinsics.checkNotNullParameter(anchorBounds, "anchorBounds");
        Intrinsics.checkNotNullParameter(layoutDirection, "layoutDirection");
        int x = anchorBounds.getRight();
        if (IntSize.m7714getWidthimpl(popupContentSize) + x > IntSize.m7714getWidthimpl(windowSize) && (x = anchorBounds.getLeft() - IntSize.m7714getWidthimpl(popupContentSize)) < 0) {
            x = anchorBounds.getLeft() + ((anchorBounds.getWidth() - IntSize.m7714getWidthimpl(popupContentSize)) / 2);
        }
        int y = (anchorBounds.getTop() - IntSize.m7713getHeightimpl(popupContentSize)) - this.tooltipAnchorPadding;
        if (y < 0) {
