...
public final class ContentAlpha {
    ...
    private final float contentAlpha(float highContrastAlpha, float lowContrastAlpha, Composer $composer, int $changed) {
        ...
        long contentColor = ((Color) consume).m4973unboximpl();
        float f = (!MaterialTheme.INSTANCE.getColors($composer, 6).isLight() ? ((double) ColorKt.m5015luminance8_81llA(contentColor)) >= 0.5d : ((double) ColorKt.m5015luminance8_81llA(contentColor)) <= 0.5d) ? lowContrastAlpha : highContrastAlpha;
        if (ComposerKt.isTraceInProgress()) {
            ComposerKt.traceEventEnd();
        }
        $composer.endReplaceableGroup();
