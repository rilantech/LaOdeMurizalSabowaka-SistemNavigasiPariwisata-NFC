...
final class DefaultElevationOverlay implements ElevationOverlay {
    ...
    public long m2985apply7g2Lkgo(long color, float elevation, Composer $composer, int $changed) {
        ...
        if (Dp.m7553compareTo0680j_4(elevation, Dp.m7554constructorimpl((float) 0)) <= 0 || colors.isLight()) {
            foregroundColor = color;
        } else {
            foregroundColor = ColorKt.m5008compositeOverOWjLjI(ElevationOverlayKt.m3016calculateForegroundColorCLU3JFs(color, elevation, $composer, ($changed & 14) | ($changed & 112)), color);
        }
        if (ComposerKt.isTraceInProgress()) {
