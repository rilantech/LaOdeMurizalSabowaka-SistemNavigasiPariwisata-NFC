...
public final class LazyListAnimateScrollScope implements LazyAnimateScrollScope {
    ...
    public LazyListAnimateScrollScope(LazyListState state2) {
    ...
    public Integer getTargetItemOffset(int index) {
        ...
        while (true) {
            ...
            if (((LazyListItemInfo) it$iv).getIndex() == index) {
                break;
            }
            index$iv$iv++;
        }
    ...
    public float expectedDistanceTo(int index, int targetScrollOffset) {
        ...
        int averageSize = (sum$iv / $this$fastSumBy$iv.size()) + layoutInfo.getMainAxisItemSpacing();
        int indexesDiff = index - getFirstVisibleItemIndex();
        int coercedOffset = Math.min(Math.abs(targetScrollOffset), averageSize);
        if (targetScrollOffset < 0) {
            coercedOffset *= -1;
        }
        return (((float) (averageSize * indexesDiff)) + ((float) coercedOffset)) - ((float) getFirstVisibleItemScrollOffset());
    }
