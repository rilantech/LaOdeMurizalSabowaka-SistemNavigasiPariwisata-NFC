...
public final class LazyGridMeasuredLine {
    ...
    public LazyGridMeasuredLine(int index2, LazyGridMeasuredItem[] items2, LazyGridSlots slots2, List<GridItemSpan> spans2, boolean isVertical2, int mainAxisSpacing2) {
        LazyGridMeasuredItem[] lazyGridMeasuredItemArr = items2;
        LazyGridSlots lazyGridSlots = slots2;
        List<GridItemSpan> list = spans2;
        Intrinsics.checkNotNullParameter(lazyGridMeasuredItemArr, "items");
        Intrinsics.checkNotNullParameter(lazyGridSlots, "slots");
    ...
    public final LazyGridMeasuredItem[] position(int offset, int layoutWidth, int layoutHeight) {
        ...
        while (i < length) {
            ...
            LazyGridMeasuredItem item = lazyGridMeasuredItemArr[i];
            int span = GridItemSpan.m2404getCurrentLineSpanimpl(this.spans.get(index$iv).m2407unboximpl());
            int startSlot = usedSpan;
            int i2 = this.slots.getPositions()[startSlot];
            boolean z = this.isVertical;
            item.position(offset, i2, layoutWidth, layoutHeight, z ? this.index : startSlot, z ? startSlot : this.index);
