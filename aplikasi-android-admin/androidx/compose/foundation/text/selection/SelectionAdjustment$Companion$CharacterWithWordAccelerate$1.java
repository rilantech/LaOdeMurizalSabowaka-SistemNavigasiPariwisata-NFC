...
public final class SelectionAdjustment$Companion$CharacterWithWordAccelerate$1 implements SelectionAdjustment {
    ...
    public long m2773adjustZXO7KMw(TextLayoutResult textLayoutResult, long newRawSelectionRange, int previousHandleOffset, boolean isStartHandle, TextRange previousSelectionRange) {
        int end;
        int start;
        boolean z = isStartHandle;
        Intrinsics.checkNotNullParameter(textLayoutResult, "textLayoutResult");
        if (previousSelectionRange == null) {
            return SelectionAdjustment.Companion.$$INSTANCE.getWord().m2769adjustZXO7KMw(textLayoutResult, newRawSelectionRange, previousHandleOffset, isStartHandle, previousSelectionRange);
        }
        if (TextRange.m7029getCollapsedimpl(newRawSelectionRange)) {
            return SelectionAdjustmentKt.ensureAtLeastOneChar(textLayoutResult.getLayoutInput().getText().getText(), TextRange.m7035getStartimpl(newRawSelectionRange), StringsKt.getLastIndex(textLayoutResult.getLayoutInput().getText()), z, TextRange.m7034getReversedimpl(previousSelectionRange.m7039unboximpl()));
        }
        if (z) {
            start = updateSelectionBoundary(textLayoutResult, TextRange.m7035getStartimpl(newRawSelectionRange), previousHandleOffset, TextRange.m7035getStartimpl(previousSelectionRange.m7039unboximpl()), TextRange.m7030getEndimpl(newRawSelectionRange), true, TextRange.m7034getReversedimpl(newRawSelectionRange));
            end = TextRange.m7030getEndimpl(newRawSelectionRange);
        } else {
            start = TextRange.m7035getStartimpl(newRawSelectionRange);
            end = updateSelectionBoundary(textLayoutResult, TextRange.m7030getEndimpl(newRawSelectionRange), previousHandleOffset, TextRange.m7030getEndimpl(previousSelectionRange.m7039unboximpl()), TextRange.m7035getStartimpl(newRawSelectionRange), false, TextRange.m7034getReversedimpl(newRawSelectionRange));
        }
    ...
    private final int snapToWordBoundary(TextLayoutResult textLayoutResult, int newRawOffset, int currentLine, int otherBoundaryOffset, boolean isStart, boolean isReversed) {
        int start;
        int end;
        long wordBoundary = textLayoutResult.m7009getWordBoundaryjx7JFs(newRawOffset);
        if (textLayoutResult.getLineForOffset(TextRange.m7035getStartimpl(wordBoundary)) == currentLine) {
            start = TextRange.m7035getStartimpl(wordBoundary);
        } else {
            start = textLayoutResult.getLineStart(currentLine);
        }
        if (textLayoutResult.getLineForOffset(TextRange.m7030getEndimpl(wordBoundary)) == currentLine) {
            end = TextRange.m7030getEndimpl(wordBoundary);
        } else {
    ...
    private final boolean isAtWordBoundary(TextLayoutResult $this$isAtWordBoundary, int offset) {
        long wordBoundary = $this$isAtWordBoundary.m7009getWordBoundaryjx7JFs(offset);
        return offset == TextRange.m7035getStartimpl(wordBoundary) || offset == TextRange.m7030getEndimpl(wordBoundary);
    }
