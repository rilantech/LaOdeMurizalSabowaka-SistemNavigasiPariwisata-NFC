...
public final class SelectionManagerKt {
    ...
    private static final long calculateSelectionMagnifierCenterAndroid_O0kMr_c$getMagnifierCenter(SelectionManager $manager, long $magnifierSize, Selection.AnchorInfo anchor, boolean isStartHandle) {
        ...
        Offset r6 = $manager.m2804getCurrentDragPosition_m7T9E();
        Intrinsics.checkNotNull(r6);
        float dragX = Offset.m4713getXimpl(selectableCoordinates.m6361localPositionOfR5De75A(containerCoordinates, r6.m4723unboximpl()));
        long line = selectable.m2766getRangeOfLineContainingjx7JFs(offset);
        Rect lineMin = selectable.getBoundingBox(TextRange.m7033getMinimpl(line));
        Rect lineMax = selectable.getBoundingBox(RangesKt.coerceAtLeast(TextRange.m7032getMaximpl(line) - 1, TextRange.m7033getMinimpl(line)));
        float maxX = RangesKt.coerceIn(dragX, Math.min(lineMin.getLeft(), lineMax.getLeft()), Math.max(lineMin.getRight(), lineMax.getRight()));
        if (Math.abs(dragX - maxX) > ((float) (IntSize.m7714getWidthimpl($magnifierSize) / 2))) {
            return Offset.Companion.m4728getUnspecifiedF1C5BW0();
        }
        return containerCoordinates.m6361localPositionOfR5De75A(selectableCoordinates, OffsetKt.Offset(maxX, Offset.m4714getYimpl(selectable.getBoundingBox(offset).m4743getCenterF1C5BW0())));
    }

    /* renamed from: calculateSelectionMagnifierCenterAndroid-O0kMr_c  reason: not valid java name */
    public static final long m2818calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager manager, long magnifierSize) {
    ...
    public static final Rect visibleBounds(LayoutCoordinates $this$visibleBounds) {
        ...
        return RectKt.m4751Rect0a9Yr6o($this$visibleBounds.m6365windowToLocalMKHz9U(boundsInWindow.m4748getTopLeftF1C5BW0()), $this$visibleBounds.m6365windowToLocalMKHz9U(boundsInWindow.m4742getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA  reason: not valid java name */
    public static final boolean m2819containsInclusiveUv8p0NA(Rect $this$containsInclusive_u2dUv8p0NA, long offset) {
