...
public final class TextSelectionDelegateKt {
    public static final long getSelectionHandleCoordinates(TextLayoutResult textLayoutResult, int offset, boolean isStart, boolean areHandlesCrossed) {
        ...
        return OffsetKt.Offset(getHorizontalPosition(textLayoutResult, offset, isStart, areHandlesCrossed), textLayoutResult.getLineBottom(textLayoutResult.getLineForOffset(offset)));
    }
    ...
    public static final float getHorizontalPosition(TextLayoutResult $this$getHorizontalPosition, int offset, boolean isStart, boolean areHandlesCrossed) {
        ...
        if ((!isStart || areHandlesCrossed) && (isStart || !areHandlesCrossed)) {
            offsetToCheck = Math.max(offset - 1, 0);
        } else {
            offsetToCheck = offset;
        }
        if ($this$getHorizontalPosition.getBidiRunDirection(offsetToCheck) == $this$getHorizontalPosition.getParagraphDirection(offset)) {
            z = true;
        }
        return $this$getHorizontalPosition.getHorizontalPosition(offset, z);
    }
}
