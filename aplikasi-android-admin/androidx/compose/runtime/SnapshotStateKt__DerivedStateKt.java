...
final /* synthetic */ class SnapshotStateKt__DerivedStateKt {
    ...
    private static final <R> R notifyObservers$SnapshotStateKt__DerivedStateKt(DerivedState<?> derivedState, Function0<? extends R> block) {
        ...
        MutableVector this_$iv = derivedStateObservers2;
        int size$iv = this_$iv.getSize();
        if (size$iv > 0) {
            int i$iv = 0;
            Object[] content$iv = this_$iv.getContent();
            do {
                ((DerivedStateObserver) content$iv[i$iv]).start(derivedState);
                i$iv++;
            } while (i$iv < size$iv);
        }
        try {
            ...
            MutableVector this_$iv2 = derivedStateObservers2;
            int size$iv2 = this_$iv2.getSize();
            if (size$iv2 > 0) {
                int i$iv2 = 0;
                Object[] content$iv2 = this_$iv2.getContent();
                do {
                    ((DerivedStateObserver) content$iv2[i$iv2]).done(derivedState);
                    i$iv2++;
            ...
            InlineMarker.finallyStart(1);
            MutableVector this_$iv3 = derivedStateObservers2;
            int size$iv3 = this_$iv3.getSize();
            if (size$iv3 > 0) {
                int i$iv3 = 0;
                Object[] content$iv3 = this_$iv3.getContent();
                do {
                    ((DerivedStateObserver) content$iv3[i$iv3]).done(derivedState);
                    i$iv3++;
    ...
    public static final <R> void observeDerivedStateRecalculations(DerivedStateObserver observer, Function0<? extends R> block) {
        ...
        MutableVector this_$iv = SnapshotStateKt.derivedStateObservers();
        try {
            this_$iv.add(observer);
            block.invoke();
        } finally {
            InlineMarker.finallyStart(1);
            this_$iv.removeAt(this_$iv.getSize() - 1);
            InlineMarker.finallyEnd(1);
        }
    }
}
