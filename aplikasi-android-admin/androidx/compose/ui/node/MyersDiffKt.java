...
public final class MyersDiffKt {
    private static final IntStack calculateDiff(int oldSize, int newSize, DiffCallback cb) {
        ...
        int[] forward = CenteredArray.m6489constructorimpl(new int[((oldEnd * 2) + 1)]);
        int[] backward = CenteredArray.m6489constructorimpl(new int[((oldEnd * 2) + 1)]);
        int[] snake2 = Snake.m6729constructorimpl(new int[5]);
        while (stack.isNotEmpty()) {
            ...
            int newEnd2 = newEnd;
            int[] snake3 = snake2;
            if (m6614midPointq5eDKzI(oldStart, oldEnd2, newStart, newEnd, cb, iArr, backward, snake2)) {
                if (Snake.m6732getDiagonalSizeimpl(snake3) > 0) {
                    snake = snake3;
                    Snake.m6727addDiagonalToStackimpl(snake, diagonals);
                } else {
                    snake = snake3;
                }
                stack.pushRange(oldStart2, Snake.m6737getStartXimpl(snake), newStart2, Snake.m6738getStartYimpl(snake));
                stack.pushRange(Snake.m6733getEndXimpl(snake), oldEnd3, Snake.m6734getEndYimpl(snake), newEnd2);
    ...
    private static final boolean m6614midPointq5eDKzI(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int[] snake) {
        int i = oldStart;
        int i2 = oldEnd;
        int oldSize = i2 - i;
        ...
        CenteredArray.m6495setimpl(forward, 1, i);
        CenteredArray.m6495setimpl(backward, 1, i2);
        int d = 0;
        while (d < max) {
            int d2 = d;
            if (m6613forward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake) || m6612backward4l5_RBY(oldStart, oldEnd, newStart, newEnd, cb, forward, backward, d2, snake)) {
                return true;
            }
            d = d2 + 1;
            int[] iArr = forward;
    ...
    private static final boolean m6613forward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int x;
        int startX;
        int i = oldEnd;
        int i2 = newEnd;
        ...
        int k = -i3;
        while (k <= i3) {
            if (k == (-i3) || (k != i3 && CenteredArray.m6492getimpl(iArr, k + 1) > CenteredArray.m6492getimpl(iArr, k - 1))) {
                startX = CenteredArray.m6492getimpl(iArr, k + 1);
                x = startX;
            } else {
                startX = CenteredArray.m6492getimpl(iArr, k - 1);
                x = startX + 1;
            ...
            CenteredArray.m6495setimpl(iArr, k, x);
            if (checkForSnake) {
                int backwardsK = delta - k;
                if (backwardsK < (-i3) + i4 || backwardsK > i3 - 1) {
                    int[] iArr2 = backward;
                } else if (CenteredArray.m6492getimpl(backward, backwardsK) <= x) {
    ...
    private static final boolean m6612backward4l5_RBY(int oldStart, int oldEnd, int newStart, int newEnd, DiffCallback cb, int[] forward, int[] backward, int d, int[] snake) {
        int x;
        int startX;
        int i = oldStart;
        int i2 = newStart;
        ...
        for (int k = -i3; k <= i3; k += 2) {
            ...
            if (checkForSnake) {
                ...
                if (forwardsK < (-i3) || forwardsK > i3 || CenteredArray.m6492getimpl(forward, forwardsK) < x) {
                } else {
                    int i4 = forwardsK;
                    int i5 = y2;
