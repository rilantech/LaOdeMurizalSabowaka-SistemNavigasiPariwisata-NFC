...
public final class MultiParagraphKt {
    public static final int findParagraphByIndex(List<ParagraphInfo> paragraphInfoList, int index) {
        ...
        List $this$fastBinarySearch$iv = paragraphInfoList;
        int low$iv = 0;
        int high$iv = $this$fastBinarySearch$iv.size() - 1;
        while (low$iv <= high$iv) {
            int mid$iv = (low$iv + high$iv) >>> 1;
            ParagraphInfo paragraphInfo = $this$fastBinarySearch$iv.get(mid$iv);
            if (paragraphInfo.getStartIndex() > index) {
                i = 1;
            } else if (paragraphInfo.getEndIndex() <= index) {
                i = -1;
            } else {
                i = 0;
            }
    ...
    public static final int findParagraphByY(List<ParagraphInfo> paragraphInfoList, float y) {
        ...
        while (low$iv <= high$iv) {
            ...
            if (paragraphInfo.getTop() > y) {
                i = 1;
            } else if (paragraphInfo.getBottom() <= y) {
                i = -1;
            } else {
                i = 0;
            }
    ...
    public static final int findParagraphByLineIndex(List<ParagraphInfo> paragraphInfoList, int lineIndex) {
        ...
        while (low$iv <= high$iv) {
            ...
            if (paragraphInfo.getStartLineIndex() > lineIndex) {
                i = 1;
            } else if (paragraphInfo.getEndLineIndex() <= lineIndex) {
                i = -1;
            } else {
                i = 0;
            }
    ...
    private static final <T> int fastBinarySearch(List<? extends T> $this$fastBinarySearch, Function1<? super T, Integer> comparison) {
        int low = 0;
        int high = $this$fastBinarySearch.size() - 1;
        while (low <= high) {
            int mid = (low + high) >>> 1;
            int cmp = comparison.invoke($this$fastBinarySearch.get(mid)).intValue();
            if (cmp < 0) {
                low = mid + 1;
            } else if (cmp <= 0) {
                return mid;
