...
public final class FocusTargetNode extends Modifier.Node implements FocusTargetModifierNode, ObserverModifierNode, ModifierLocalModifierNode {
    ...
    public final FocusProperties fetchFocusProperties$ui_release() {
        ...
        int type$iv5 = NodeKind.m6666constructorimpl(2048);
        int untilType$iv4 = NodeKind.m6666constructorimpl(1024);
        int i5 = false;
    ...
    public final void m4659fetchCustomEnteraToIllA$ui_release(int focusDirection, Function1<? super FocusRequester, Unit> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        if (!this.isProcessingCustomEnter) {
            this.isProcessingCustomEnter = true;
            try {
                FocusRequester it = fetchFocusProperties$ui_release().getEnter().invoke(FocusDirection.m4626boximpl(focusDirection));
    ...
    public final void m4660fetchCustomExitaToIllA$ui_release(int focusDirection, Function1<? super FocusRequester, Unit> block) {
        Intrinsics.checkNotNullParameter(block, "block");
        if (!this.isProcessingCustomExit) {
            this.isProcessingCustomExit = true;
            try {
                FocusRequester it = fetchFocusProperties$ui_release().getExit().invoke(FocusDirection.m4626boximpl(focusDirection));
                if (it != FocusRequester.Companion.getDefault()) {
                    block.invoke(it);
                }
            } finally {
    ...
    public final void scheduleInvalidationForFocusEvents$ui_release() {
        ...
        int kind$iv = NodeKind.m6666constructorimpl(4096);
        ...
        int i6 = 1024;
        int mask$iv5 = NodeKind.m6666constructorimpl(4096) | NodeKind.m6666constructorimpl(1024);
                ...
                if ((layout$iv.getNodes$ui_release().getHead$ui_release().getAggregateChildKindSet$ui_release() & mask$iv5) != 0) {
                    while (node$iv != null) {
                        if ((node$iv.getKindSet$ui_release() & mask$iv2) != 0) {
                            ...
                            if (((it.getKindSet$ui_release() & NodeKind.m6666constructorimpl(i6)) != 0 ? 1 : i3) == 0 && it.isAttached()) {
                                int kind$iv2 = NodeKind.m6666constructorimpl(4096);
