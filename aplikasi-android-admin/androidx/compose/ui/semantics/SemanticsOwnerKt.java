...
public final class SemanticsOwnerKt {
    ...
    public static final Map<Integer, SemanticsNode> getAllSemanticsNodesToMap(SemanticsOwner $this$getAllSemanticsNodesToMap, boolean useUnmergedTree) {
        ...
        getAllSemanticsNodesToMap$findAllSemanticNodesRecursive(nodes, useUnmergedTree ? $this$getAllSemanticsNodesToMap.getUnmergedRootSemanticsNode() : $this$getAllSemanticsNodesToMap.getRootSemanticsNode());
        return nodes;
    }

    private static final void getAllSemanticsNodesToMap$findAllSemanticNodesRecursive(Map<Integer, SemanticsNode> nodes, SemanticsNode currentNode) {
        nodes.put(Integer.valueOf(currentNode.getId()), currentNode);
        List $this$fastForEach$iv = currentNode.getChildren();
        int size = $this$fastForEach$iv.size();
        for (int index$iv = 0; index$iv < size; index$iv++) {
            getAllSemanticsNodesToMap$findAllSemanticNodesRecursive(nodes, $this$fastForEach$iv.get(index$iv));
        }
    }
}
