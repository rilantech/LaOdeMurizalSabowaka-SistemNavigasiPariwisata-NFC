    ...
    private static final boolean calculateIfHorizontallyStacked(List<SemanticsNode> items) {
        ...
        if ($this$fastZipWithNext$iv.size() == 0 || $this$fastZipWithNext$iv.size() == 1) {
            ...
            for (int i$iv = 0; i$iv < lastIndex; i$iv++) {
                ...
                result$iv.add(Offset.m4702boximpl(OffsetKt.Offset(Math.abs(Offset.m4713getXimpl(el1.getBoundsInRoot().m4743getCenterF1C5BW0()) - Offset.m4713getXimpl(el2.getBoundsInRoot().m4743getCenterF1C5BW0())), Math.abs(Offset.m4714getYimpl(el1.getBoundsInRoot().m4743getCenterF1C5BW0()) - Offset.m4714getYimpl(el2.getBoundsInRoot().m4743getCenterF1C5BW0())))));
                current$iv = next$iv;
            }
        }
        List deltas = result$iv;
        if (deltas.size() == 1) {
            j = ((Offset) CollectionsKt.first(deltas)).m4723unboximpl();
        } else {
            ...
            if (!$this$fastReduce$iv.isEmpty()) {
                ...
                if (1 <= lastIndex2) {
                    while (true) {
                        accumulator$iv = Offset.m4702boximpl(Offset.m4718plusMKHz9U(((Offset) accumulator$iv).m4723unboximpl(), ((Offset) $this$fastReduce$iv.get(i$iv2)).m4723unboximpl()));
                        if (i$iv2 == lastIndex2) {
                            break;
                        }
                        i$iv2++;
                ...
                j = ((Offset) accumulator$iv).m4723unboximpl();
            } else {
                throw new UnsupportedOperationException("Empty collection can't be reduced.");
            }
        }
        if (Offset.m4704component2impl(j) < Offset.m4703component1impl(j)) {
            return true;
        }
        return false;
    }
