...
public final class AndroidCanvas implements Canvas {
    ...
    public void m4809concat58bKbWc(float[] matrix) {
        Intrinsics.checkNotNullParameter(matrix, "matrix");
        if (!MatrixKt.m5216isIdentity58bKbWc(matrix)) {
            Matrix frameworkMatrix = new Matrix();
            AndroidMatrixConversions_androidKt.m4824setFromEL8BTi8(frameworkMatrix, matrix);
            this.internalCanvas.concat(frameworkMatrix);
        }
    }

    /* renamed from: clipRect-N_I0leg  reason: not valid java name */
    public void m4808clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m4817toRegionOp7u2Bmg(clipOp));
    }

    /* renamed from: clipPath-mtrdD-E  reason: not valid java name */
    public void m4807clipPathmtrdDE(Path path, int clipOp) {
        Intrinsics.checkNotNullParameter(path, "path");
        Canvas canvas = this.internalCanvas;
        Path $this$asAndroidPath$iv = path;
        if ($this$asAndroidPath$iv instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) $this$asAndroidPath$iv).getInternalPath(), m4817toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m4817toRegionOp7u2Bmg(int $this$toRegionOp_u2d_u2d7u2Bmg) {
        if (ClipOp.m4947equalsimpl0($this$toRegionOp_u2d_u2d7u2Bmg, ClipOp.Companion.m4951getDifferencertfAjoo())) {
    ...
    public void m4813drawLineWko1d7g(long p1, long p2, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawLine(Offset.m4713getXimpl(p1), Offset.m4714getYimpl(p1), Offset.m4713getXimpl(p2), Offset.m4714getYimpl(p2), paint.asFrameworkPaint());
    }
    ...
    public void m4810drawCircle9KIMszo(long center, float radius, Paint paint) {
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawCircle(Offset.m4713getXimpl(center), Offset.m4714getYimpl(center), radius, paint.asFrameworkPaint());
    }
    ...
    public void m4811drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        Intrinsics.checkNotNullParameter(image, "image");
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m4713getXimpl(topLeftOffset), Offset.m4714getYimpl(topLeftOffset), paint.asFrameworkPaint());
    }

    /* renamed from: drawImageRect-HPBpro0  reason: not valid java name */
    public void m4812drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        $this$drawImageRect_HPBpro0_u24lambda_u240.left = IntOffset.m7672getXimpl(srcOffset);
        $this$drawImageRect_HPBpro0_u24lambda_u240.top = IntOffset.m7673getYimpl(srcOffset);
        $this$drawImageRect_HPBpro0_u24lambda_u240.right = IntOffset.m7672getXimpl(srcOffset) + IntSize.m7714getWidthimpl(srcSize);
        $this$drawImageRect_HPBpro0_u24lambda_u240.bottom = IntOffset.m7673getYimpl(srcOffset) + IntSize.m7713getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Rect $this$drawImageRect_HPBpro0_u24lambda_u241 = rect2;
        $this$drawImageRect_HPBpro0_u24lambda_u241.left = IntOffset.m7672getXimpl(dstOffset);
        $this$drawImageRect_HPBpro0_u24lambda_u241.top = IntOffset.m7673getYimpl(dstOffset);
        $this$drawImageRect_HPBpro0_u24lambda_u241.right = IntOffset.m7672getXimpl(dstOffset) + IntSize.m7714getWidthimpl(dstSize);
        $this$drawImageRect_HPBpro0_u24lambda_u241.bottom = IntOffset.m7673getYimpl(dstOffset) + IntSize.m7713getHeightimpl(dstSize);
        Unit unit2 = Unit.INSTANCE;
        canvas.drawBitmap(asAndroidBitmap, rect, rect2, paint.asFrameworkPaint());
    }

    /* renamed from: drawPoints-O7TthRY  reason: not valid java name */
    public void m4814drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
        if (PointMode.m5281equalsimpl0(pointMode, PointMode.Companion.m5285getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m5281equalsimpl0(pointMode, PointMode.Companion.m5287getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m5281equalsimpl0(pointMode, PointMode.Companion.m5286getPointsr_lszbg())) {
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int index$iv = 0; index$iv < size; index$iv++) {
            long point = $this$fastForEach$iv.get(index$iv).m4723unboximpl();
            this.internalCanvas.drawPoint(Offset.m4713getXimpl(point), Offset.m4714getYimpl(point), paint.asFrameworkPaint());
        }
    }
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                long p1 = points.get(i).m4723unboximpl();
                long p2 = points.get(i + 1).m4723unboximpl();
                this.internalCanvas.drawLine(Offset.m4713getXimpl(p1), Offset.m4714getYimpl(p1), Offset.m4713getXimpl(p2), Offset.m4714getYimpl(p2), frameworkPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void m4815drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
        Intrinsics.checkNotNullParameter(points, "points");
        Intrinsics.checkNotNullParameter(paint, "paint");
        if (points.length % 2 != 0) {
            throw new IllegalArgumentException("points must have an even number of values");
        } else if (PointMode.m5281equalsimpl0(pointMode, PointMode.Companion.m5285getLinesr_lszbg())) {
    ...
    public void m4816drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        Intrinsics.checkNotNullParameter(vertices, "vertices");
        Intrinsics.checkNotNullParameter(paint, "paint");
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m4860toAndroidVertexModeJOOmi9M(vertices.m5401getVertexModec2xauaI()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.asFrameworkPaint());
    }
}
