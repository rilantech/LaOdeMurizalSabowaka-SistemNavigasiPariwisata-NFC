...
final class RenderEffectVerificationHelper {
    ...
    public final RenderEffect m5294createBlurEffect8A3gB4(RenderEffect inputRenderEffect, float radiusX, float radiusY, int edgeTreatment) {
        if (inputRenderEffect == null) {
            RenderEffect createBlurEffect = RenderEffect.createBlurEffect(radiusX, radiusY, AndroidTileMode_androidKt.m4859toAndroidTileMode0vamqd0(edgeTreatment));
            Intrinsics.checkNotNullExpressionValue(createBlurEffect, "{\n            android.gr…)\n            )\n        }");
            return createBlurEffect;
        }
        RenderEffect createBlurEffect2 = RenderEffect.createBlurEffect(radiusX, radiusY, inputRenderEffect.asAndroidRenderEffect(), AndroidTileMode_androidKt.m4859toAndroidTileMode0vamqd0(edgeTreatment));
    ...
    public final RenderEffect m5295createOffsetEffectUv8p0NA(RenderEffect inputRenderEffect, long offset) {
        if (inputRenderEffect == null) {
            RenderEffect createOffsetEffect = RenderEffect.createOffsetEffect(Offset.m4713getXimpl(offset), Offset.m4714getYimpl(offset));
            Intrinsics.checkNotNullExpressionValue(createOffsetEffect, "{\n            android.gr…et.x, offset.y)\n        }");
            return createOffsetEffect;
        }
        RenderEffect createOffsetEffect2 = RenderEffect.createOffsetEffect(Offset.m4713getXimpl(offset), Offset.m4714getYimpl(offset), inputRenderEffect.asAndroidRenderEffect());
